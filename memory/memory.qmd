...
- If this sampling is unbiased, then $\small R_{q_1} \approx 0.1\%\cdot T = 1$ and $\small R_{q_2} \approx 0.9\% \cdot T = 9.$
- The estimate based on unbiased sampling is approx. equal to the truth:
...
$$
\small r_{q_1}(\text{red hair, irish}) = \frac{1\cdot 0.01}{1\cdot 0.01 + 1/2 \cdot 0.09 + 1/2\cdot 0.01 + 0\cdot 0.9} \approx 0.167
$$
...
$$
\small r_{q_1}(\text{red hair, irish}) = \frac{1\cdot 0.01}{1\cdot 0.01 + 1/2 \cdot 0.09 + 1/2\cdot 0.01 + 0\cdot 0.89} \approx 0.167
$$

- After searching for $q_2 = (\text{other hair, irish}):$

$$
\small r_{q_2}(\text{other hair, irish}) = \frac{1\cdot 0.09}{1\cdot 0.09 + 1/2 \cdot 0.01 + 1/2\cdot 0.89 + 0\cdot 0.01} \approx 0.167
$$
...
- When cued with a feature, individuals will draw on their memory to "fill in" the group that is likely to have this feature.
  - For example, people may have certain associations in mind between what groups in society are likely to commit crimes, receive welfare, etc. 
...
- Associative (cued) recall amplifies diagnostic features of groups, leading to overestimation.
- Laboratory and field evidence support the model: memory interference and media cues shape beliefs.
- Understanding memory-based belief formation helps explain persistent social stereotypes.
::: 
